...
import PromptFilesPage from './pages/PromptFilesPage'
import GroupsPage from './pages/GroupsPage'
import { useStatus } from './hooks/useStatus'
import { useTheme } from './hooks/useTheme'

export type PageId = 'status' | 'config' | 'prompts' | 'groups'

const pageConfig: Record<PageId, { title: string; desc: string }> = {
    status: { title: '仪表盘', desc: '插件运行状态与数据概览' },
    config: { title: '插件配置', desc: '基础设置与参数配置' },
    prompts: { title: 'Prompt 文件', desc: '直接编辑人设、动态记忆和 Prompt 模板' },
...
function App() {
    ...
    const renderPage = () => {
        switch (currentPage) {
            ...
            case 'prompts': return <PromptFilesPage />
